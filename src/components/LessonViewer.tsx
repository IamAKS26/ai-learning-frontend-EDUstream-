...
export function LessonViewer({ unit, moduleId, onComplete }: Props) {
  ...
  return (
    <div className="glass-card rounded-2xl p-8 max-w-4xl mx-auto flex flex-col min-h-[600px]">
          ...
          <article className="max-w-none text-slate-700">
            <ReactMarkdown 
              ...
              components={{
                ...
                table: ({node, ...props}) => (
                  <div className="w-full overflow-x-auto mb-8 rounded-xl border border-black/5 shadow-lg">
      ...
      {/* Footer */}
      <div className="mt-8 flex justify-end pt-6 border-t border-black/5">
        <button
          onClick={() => onComplete({ timeSpent })}
          className="px-8 py-3 bg-primary text-slate-900 font-bold rounded-xl hover:bg-primary/90 transition-all flex items-center gap-2 shadow-[0_4px_12px_rgba(255,179,0,0.3)] hover:shadow-[0_6px_20px_rgba(255,179,0,0.4)] hover:-translate-y-0.5"
        >
