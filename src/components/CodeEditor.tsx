...
export function CodeEditor({ language, starterCode, taskDescription, onSubmit }: Props) {
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4">
      {/* Code Editor Area */}
      <div className="rounded-xl overflow-hidden border border-white/10 bg-[#1e1e1e] shadow-2xl relative">
        <div className="flex items-center justify-between px-4 py-2 bg-[#2d2d2d] border-b border-black/40">
          ...
          <span className="text-xs font-mono font-bold text-slate-400 uppercase tracking-widest">{language}</span>
        </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="px-6 py-3 bg-primary text-background-dark font-bold rounded-xl hover:bg-primary/90 transition-all flex items-center gap-2 shadow-[0_4px_12px_rgba(255,179,0,0.2)] disabled:opacity-50"
        >
          {loading ? (
      ...
      {review && (
        <div className="mt-12 glass-card rounded-2xl p-6 border border-primary/20 bg-primary/5 shadow-[0_0_30px_rgba(255,179,0,0.1)] slide-up fade-in transition-all">
          <div className="flex items-start gap-4 mb-6 pb-6 border-b border-white/10">
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-display font-bold text-2xl shadow-inner border border-white/10 ${
            ...
            <div>
              <h3 className="text-xl font-bold font-display text-white">AI Senior Developer Review</h3>
              <p className="text-slate-300 mt-1">{review.summary}</p>
            </div>
          </div>
                ...
                <div className="space-y-3">
                  {review.suggestions.map((sug, i) => (
                    <div key={i} className="bg-white/5 border border-white/10 p-4 rounded-xl">
                      ...
                      <p className="text-slate-400 text-sm leading-relaxed mb-3">{sug.description}</p>
            ...
            {review.bestPractice && (
              <div className="bg-primary/10 border border-primary/20 p-4 rounded-xl flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-300 italic">"{review.bestPractice}"</p>
                </div>
              </div>
            )}
          </div>
